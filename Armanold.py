...
def banner():
	clear()
	sol()
	ban=''' © Ibnz Facebook account
▄. Author :Ibnz. Wa +88013188058**.Status : ARMAN
·▀▀▀▀ .▀  ▀ ▀▀▀  ▀▀▀  ▀▀▀     ▀▀▀ ·▀▀▀▀ '''
...
def menu(my_name,my_id):
	...
	print('├──> 0. Keluar       ')
	print('|')
	_____YT__Ibnz_____ = input('\r├──> Pilih : ')
	if _____YT__Ibnz_____ in ['1']:
		...
		crack_file()
	elif _____YT__Ibnz_____ in ['5']:
		result()
	elif _____YT__Ibnz_____ in ['6']:
		memek()
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x}——> {h}Gua Ibnz\n\t{x}——> {h}baru belajar ngoding\n\t{x}——> {h}jangan di perjual belikan\n\t{x}——> {h}khusus +62\n\t{x}——> {h}Dan jangan lupa habisin kuota elu {x}')
	time.sleep(3)
	login()
